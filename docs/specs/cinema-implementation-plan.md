...
- [x] Добавить локальные Bootstrap CSS/JS файлы.
- [x] Создать fragments:
...
- [x] В навигации показывать `Регистрация` и `Вход` для гостя.
- [x] В навигации показывать имя пользователя и `Выйти` для авторизованного пользователя.
- [x] На странице покупки вывести два `select`: ряд и место.
- [x] Постеры выводить через `/files/{id}`.
...
- [x] Заполнить `README.md`:
  ...
  - контакты.
- [x] Сделать скриншоты страниц и положить их в `img`.
- [x] Добавить ссылки на скриншоты в README.
- [x] Проверить, что в репозитории нет IDE-файлов, `target`, временных БД и локальных артефактов.
...
- [x] Приложение запускается через `mvn spring-boot:run`.
- [x] Гость видит главную, кинотеку, расписание и форму покупки.
- [x] Гость при покупке попадает на `/login`.
- [x] Пользователь регистрируется и входит.
- [x] Пользователь покупает свободное место.
- [x] Повторная покупка того же места показывает страницу неудачи.
- [x] Постеры фильмов открываются через `/files/{id}`.
- [x] README содержит скриншоты и команды запуска.
